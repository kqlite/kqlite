...
func (s *Server) handleParseMessage(ctx context.Context, c *Conn, pmsg *pgproto3.Parse) error {
	...
	execDescirbe := func() (err error) {
		//log.Printf("parsed query: %#v", tree)

		stmt, err := c.db.PrepareContext(ctx, "SELECT brand, model, year FROM cars LIMIT 1")
		if err != nil {
			return fmt.Errorf("prepare: %w", err)
		}
		if rows != nil {
	...
	for {
		...
		case *pgproto3.Sync:
			...
			writeMessages(c,
				...
				&pgproto3.ParameterDescription{ParameterOIDs: []uint32{pgtype.TextOID, pgtype.TextOID}},
				//desc,
				&pgproto3.ReadyForQuery{TxStatus: 'I'})
			continue
